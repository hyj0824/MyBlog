...
[当使用git checkout命令时，Git会切换到指定的分支，但如果后面接的是文件名或路径，Git则不会切换分支，而是把文件从.git目录中复制一份覆盖到当前的工作目录。](../assets/58561_Git从入门到精通_高见龙.pdf#page=72&selection=63,6,73,16)与reset --mixed相反，它不会修改暂存区，只会修改工作区。
...
[HEAD是一个指标，指向某一个分支，通常可以把它当作“当前所在分支”来看待。](../assets/58561_Git从入门到精通_高见龙.pdf#page=77&selection=91,0,96,4)（如：refs/heads/master）。分支也是个指针，指向分支上最新的commit。例如：HEAD -> refs/heads/master -> b431fa7
